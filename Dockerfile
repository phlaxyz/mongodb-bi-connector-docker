...
RUN openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem && \
...
CMD sh -c "/mongosqld/bin/mongosqld --logPath /var/log/mongosqld.log --sslPEMKeyFile mongo.pem --mongo-uri '$MONGO_URL' --auth -u $MONGO_USER -p $MONGO_PASSWD --schemaSource $MONGO_SCHEMA --schemaMode $MONGO_SCHEMAMODE --addr 0.0.0.0:$LISTEN_PORT"
